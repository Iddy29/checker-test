...
async def _get_user_semaphore(user_id):
    async with _user_sem_lock:
        if user_id not in _user_semaphores:
            _user_semaphores[user_id] = asyncio.Semaphore(MAX_USER_CONCURRENT)
        ...
        try:
            if asyncio.iscoroutinefunction(fn):
                if gate_info["func_path"] == "b3_CCN_charge.B3_CCN":
                    ...
                    result = await asyncio.wait_for(fn(cc, mm, yy, cvv, **kwargs), timeout=60)
            else:
