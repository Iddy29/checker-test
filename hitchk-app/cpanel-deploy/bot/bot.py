...
from gateways import (
    ...
    get_user_proxy_list, get_user_proxy
)
from tools import (
    ...
    try:
        if HIT_FORWARD_GROUP:
            ...
            hit_msg = (
                ...
                f"**Response:** {response}\n"
                f"**Gateway:** {gateway}\n"
                f"**BIN:** {brand} - {bin_type} - {level}\n"
                f"**Bank:** {bank}\n"
                f"**Country:** {country} {flag}"
            )
            if user_name and user_id:
                hit_msg += f"\n**Checked By:** [{user_name}](tg://user?id={user_id})"
...
async def acc_checker_cmd(event):
    ...
    if ":" not in combo:
        return await event.reply("Invalid combo format. Use: `email:password`")
...
async def gateway_cmd(event):
    ...
    remaining_text = event.raw_text[len(match.group(0)):].strip()
...
async def generate_keys(event):
    ...
    try:
        ...
        if len(parts) != 4: return await event.reply(
            ...
            "`/key <plan> <amount> <days>`\n\n"
            "📋 **Plans:**\n"
            "• `s` — Silver\n"
            "• `g` — Gold\n\n"
            "📝 **Examples:**\n"
            "• `/key s 10 1` — 10 Silver keys for 1 day\n"
            "• `/key g 5 7` — 5 Gold keys for 7 days\n"
            "• `/key s 1 30` — 1 Silver key for 30 days"
        )
        ...
        if days > 365: return await event.reply("❌ Maximum 365 days!")
    ...
    except ValueError: await event.reply("❌ Invalid amount or days!")
    except Exception as e: await event.reply(f"❌ Error: {e}")

TIERS_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "user_tiers.json")

def save_user_tier(user_id, plan, days):
    try:
        tiers = {}
...
async def redeem_key(event):
    ...
    try:
            ...
            try:
                ...
                if expires_at:
                    ...
                    if time.time() < expiry_ts:
                        ...
                        if tier_rank.get(current_tier, 0) == tier_rank.get(plan, 0):
                            days_left = max(1, int((expiry_ts - time.time()) / 86400) + 1)
                            return await event.reply(f"❌ **Cannot Redeem!**\n\nYou already have an active **{current_tier.capitalize()}** plan ({days_left} day{'s' if days_left != 1 else ''} remaining).\n\nWait for it to expire or upgrade to a higher plan.")
            except Exception:
                pass
        await add_premium_user(event.sender_id, days)
        save_user_tier(event.sender_id, plan, days)
        ...
        await send_invoice(event.sender_id, user_name, plan, days, key)
        await send_plan_log(event.sender_id, user_name, plan, days)
    except Exception as e: await event.reply(f"❌ Error: {e}")
...
async def process_mtxt_cards(event, cards, sites):
    ...
    total = len(cards)
    await register_mass_user(user_id)
    CONCURRENT = await get_dynamic_concurrency(10)
    MAX_RETRIES = 3
    ...
    async def _process_one_with_retry(card, initial_site, slot_idx):
        ...
        for attempt in range(MAX_RETRIES + 1):
            ...
            if r_status == "dead_site":
                if attempt < MAX_RETRIES:
                    slot_status[slot_idx] = f"\U0001f504 `...{card_short}` \u2014 Dead site, retrying..."
                    await _update_status_msg()
                    current_site = await _get_different_site(tried_sites)
                    continue
                checked += 1
                errors += 1
                slot_status[slot_idx] = f"\U0001f480 `...{card_short}` \u2014 Dead site"
                return
            ...
            if "captcha" in response_lower or "cloudflare" in response_lower:
                ...
                checked += 1
                errors += 1
                slot_status[slot_idx] = f"\u26a0\ufe0f `...{card_short}` \u2014 Captcha/CF"
                return
...
async def co_cmd(event):
    ...
    url_parts = []
    for line in lines:
        if "checkout.stripe.com" in line or line.startswith("cs_live_") or line.startswith("cs_test_") or ("stripe.com" in line and ("pay" in line or "cs_" in line)):
    ...
    for eu in entity_urls:
        ...
        if candidate and ('checkout.stripe.com' in candidate or 'cs_live_' in candidate or 'cs_test_' in candidate):
    ...
    asyncio.create_task(_process_co_cards(event, cards, checkout_url, user_id))


async def _process_co_cards(event, cards, checkout_url, user_id):
        ...
        for i, (cc, mm, yy, cvv) in enumerate(cards):
            ...
            try:
                status, msg, card_info, elapsed, cached = await asyncio.wait_for(
                    stripe_co_check(cc, mm, yy, cvv, checkout_url, session_cache=session_cache),
                    timeout=60
                )
...
async def hit_cmd(event):
    ...
    asyncio.create_task(_process_hit(event, charged_ccs, checkout_url, user_id, count))

async def _process_hit(event, charged_ccs, checkout_url, user_id, requested_count):
    from gates.stripe_co import stripe_co_check
